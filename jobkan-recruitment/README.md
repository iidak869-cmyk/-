...
## 確認事項（手順19〜20・反映用リスト.xlsxへの転記）

サンプルCSVを解析して実装しましたが、以下は仮の判断で実装した箇所です。本番運用前に確認をお願いします。

- **「前日」の判定基準**: 現状は「スクリプト実行日の1日前」で固定しています（`src/excel.js` の `filterPreviousDay`）。実行タイミングが毎日決まっている前提です。
- **I列「入社種類」**: AirWORKは「応募雇用形態」列をそのまま使用。dodaのCSVには対応する固定列が見当たらなかったため、現状は空欄にしています（`src/parsers/dodaCsv.js` の `TODO(要確認)`）。
- **C列「性別」の表記統一**: AirWORKは「男性」「女性」表記、dodaは「男」「女」表記だったため、doda側を「男性」「女性」に正規化しています。
