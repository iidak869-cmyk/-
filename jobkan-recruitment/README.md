...
| 19〜20 | AirWORK/dodaのCSVから反映用リスト.xlsxへの転記（応募経路付き） | 実装済み（サンプルデータで動作確認済み。下記「確認事項」あり） |
| 21 | 応募者数CSVフォルダ内Excelからの転記 | **未実装**（サンプルファイル待ち） |
| 22 | 共有ドライブへのコピー | 実装済み |
...
- **H列「経路」**: AirWORK由来のデータは "AirWORK" 固定、doda由来のデータは "doda" 固定で入れています（応募者数CSVフォルダは「AirWORK」「doda」の2択と確認済み）。
