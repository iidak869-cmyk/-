...
| 21 | 応募者数CSVフォルダ内ファイルからの転記 | 実装済み（Instagramサンプルで動作確認済み。下記「確認事項」あり） |
| 22 | 共有ドライブへのコピー | 実装済み |
...
応募者数CSVフォルダには、LINEやInstagramなど、AirWORK/dodaとは別の集客チャネル（マーケティング
ツール）ごとのエクスポートファイル（.csv/.xlsx）が入っている前提です（Instagram向けのサンプル
ファイルで動作確認済み）。「入社種類」「希望勤務地」等はファイル内のカスタム項目名で検索するため、
チャネルによって項目が無い場合は空欄になります。

以下は仮の判断で実装した箇所です。他のチャネルのサンプルが増え次第、確認・調整をお願いします。

- **H列「経路」**: ファイル内の「シナリオ名（購入商品）」列（例: `★稼働中【WeeAre公式】Instagram`）の
  末尾（最後の「】」より後ろ）を媒体名として抽出しています（`src/parsers/applicantCsvFolder.js` の
  `extractRouteFromScenario`、`TODO(要確認)`）。シナリオ名列が無いファイルの場合はファイル名を使用します。
